...
| engine | 0.3.x | Batch results carry each machine's per-round state; the report gains 9d (field ranked by lower confidence bound, record against the anchors, occupancy). `run_batch` checkpoints each finished game to `reports/raw/<batch>.partial.jsonl` and resumes from it. |
