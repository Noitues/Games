...
from .evaluation import W, evaluate
...
class T1Greedy(Policy):
    ...
    def __init__(self, seed: int, temperature: float = 0.0, config=None):
        ...
        self.weights = dict(self.__class__.weights)
        self.weights.update((self.config or {}).get("weights", {}))

    def value(self, state) -> float:
        return evaluate(state, self.team, self.weights)
